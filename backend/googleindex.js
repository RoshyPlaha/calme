...
function listEvents(auth) {
  ...
  calendar.events.list({
    ...
    if (events.length) {
      ...
      const filtered = findCalMeEvents(events);
      filtered.forEach((event) => {
        // const start = event.start.dateTime || event.start.date;
        // console.log(`${start} - ${event.summary}`);
        console.log(JSON.stringify(event.organizer.email));
        convertDateToCron(event.start.dateTime);
      });
    } else {
...
const findCalMeEvents = (events) => {
    return events.filter((event) => event.description.includes("#calme") || event.summary.includes("#calme"));
}
